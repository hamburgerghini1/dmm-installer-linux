...
# Detect Steam library containing Project DIVA
...
# Minimum supported Wine version for running the installer
min_wine_ver="8.26"
if [ "$(printf '%s\n' "$min_wine_ver" "$wine_ver" | sort -V | head -n1)" != "$min_wine_ver" ]; then
    echo "Wine version is unsupported, please upgrade to wine version $min_wine_ver or later. You may need to install wine-staging package."
    exit 1
fi
...
# If Wine is older than 9.0, install extra .NET versions
min_dotnet_wine_ver="9.0"
if [ "$(printf '%s\n' "$min_dotnet_wine_ver" "$wine_ver" | sort -V | head -n1)" = "$wine_ver" ] && [ "$wine_ver" != "$min_dotnet_wine_ver" ]; then
